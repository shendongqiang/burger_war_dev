...
class AllSensorBot(object):
    def __init__(self, 
        ...
        # pit dircetion publisher
        self.pitDir_pub = rospy.Publisher('pit_direction', String,queue_size=1)
...
#    bot.strategy()
    ...
    pitDir_pub = rospy.Publisher('pit_direction', String,queue_size=1)
...
#    judge_th = 1000000
    judge_th = 10000
    pitDirection = "notexist" #notexit,left,right,front
    pitDirectionCount = 0
    searchPitResult = "searching" #fail,searching,found
...
#      upper = np.array([60, 255, 255]) # green
	...
	mask_sum = mask_image.sum()
        ...
        if abs(diff_pix) < 320 and diff_pix > 30:
            #anglular_z = -0.3
            anglular_z = 0.0
        elif abs(diff_pix) < 320 and diff_pix < -30:
            #anglular_z = 0.3
            anglular_z = 0.0
        if abs(diff_pix) < 320 and diff_pix <= 30:
            anglular_z = +0.3
        ...
        searchPitResult = rospy.get_param("searchPitResult")
        print("searchPitResult = %s" %searchPitResult)
        ...
        if count > count_th1:
            ...
            #敵のサーチ結果をResultに設定する
            if searchPitResult != "found":
                if (anglular_z == 0.0):
                    rospy.set_param('searchPitResult','found')
                    #actionModeの遷移をpatrol_courceノードに任せる
                    #rospy.set_param('actionMode','searchPit')

                else:
                    rospy.set_param('searchPitResult','searching')
        elif count <= 0:
             rospy.set_param('searchPitResult','fail')
             #actionModeの遷移をpatrol_courceノードに任せる
        ...
        #敵のいる方向をパブリッシュする
        pitDir_pub.publish(pitDirection)
