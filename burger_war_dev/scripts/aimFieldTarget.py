...
class AllSensorBot(object):
    def __init__(self, 
        ...
        # fieldTarget dircetion publisher
        self.fieldTargetDir_pub = rospy.Publisher('fieldTarget_direction', String,queue_size=1)
...
if __name__ == '__main__':
    rospy.init_node('aimFieldTarget')
...
#    bot.strategy()
    ...
    # field target directory publisher
    fieldTargetDir_pub = rospy.Publisher('fieldTarget_direction', String,queue_size=1)
...
#    judge_th = 1000000
    judge_th = 10000
    fieldTargetDirection = "notexist" #notexit,left,right,front
    fieldTargetDirectionCount = 0
    aimFieldTargetResult = "searching" #fail,searching,found
...
#       upper = np.array([60, 255, 255]) # green
	...
	mask_sum = mask_image.sum()
        ...
        aimFieldTargetResult = rospy.get_param("aimFieldTargetResult")
        print("aimFieldTargetResult = %s" %aimFieldTargetResult)

        #3/13 shen
        #敵のいる方向を判別する
        if cx > 0:
            fieldTargetDirection = "right"
            fieldTargetDirectionCount += 1
        elif cx < 0:
            fieldTargetDirection = "left"
            fieldTargetDirectionCount -= 1
        else :
            fieldTargetDirection = "notexist"
        ...
        if count > count_th1:
            ...
            #敵のいる方向を判別する：前方方向
            fieldTargetDirection = "front"

            #敵のサーチ結果をResultに設定する
            if aimFieldTargetResult != "found":
                if (anglular_z == 0.0):
                    rospy.set_param('aimFieldTargetResult','found')
                    #actionModeの遷移をpatrol_courceノードに任せる
                    #rospy.set_param('actionMode','aimFieldTarget')

                else:
                    rospy.set_param('aimFieldTargetResult','searching')
        elif count <= 0:
             rospy.set_param('aimFieldTargetResult','fail')
             #actionModeの遷移をpatrol_courceノードに任せる
        ...
        #敵のいる方向をパブリッシュする
        fieldTargetDir_pub.publish(fieldTargetDirection)
