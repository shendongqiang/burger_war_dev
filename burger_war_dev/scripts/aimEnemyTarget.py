...
class AllSensorBot(object):
    def __init__(self, 
        ...
        self.enemyTargetDir_pub = rospy.Publisher('enemyTarget_direction', String,queue_size=1)
...
if __name__ == '__main__':
    rospy.init_node('aimEnemyTarget')
...
#    bot.strategy()
    ...
    # enemyTarget direction publisher
    enemyTargetDir_pub = rospy.Publisher('enemyTarget_direction', String,queue_size=1)
...
#    judge_th = 1000000
    judge_th = 100
    enemyTargetDirection = "notexist" #notexit,left,right,front
    enemyTargetDirectionCount = 0
    aimEnemyTargetResult = "searching" #fail,searching,found
...
#       upper = np.array([140, 255, 255]) # blue
	...
	mask_sum = mask_image.sum()
        ...
        aimEnemyTargetResult = rospy.get_param("aimEnemyTargetResult")
        print("aimEnemyTargetResult = %s" %aimEnemyTargetResult)

        #3/13 shen
        #敵のいる方向を判別する
        if cx > 0:
            enemyTargetDirection = "right"
            enemyTargetDirectionCount += 1
        elif cx < 0:
            enemyTargetDirection = "left"
            enemyTargetDirectionCount -= 1
        else :
            enemyTargetDirection = "notexist"
        ...
        if count > count_th1:
            ...
            #敵のいる方向を判別する：前方方向
            enemyTargetDirection = "front"

            #敵のサーチ結果をResultに設定する
            if aimEnemyTargetResult != "found":

                rospy.set_param('aimEnemyTargetResult','found')
                #actionModeの遷移をpatrol_courceノードに任せる
                #rospy.set_param('actionMode','aimEnemyTarget')
             ...
             count=0
             rospy.set_param('aimEnemyTargetResult','fail')
             #actionModeの遷移をpatrol_courceノードに任せる
        ...
        #敵のいる方向をパブリッシュする
        enemyTargetDir_pub.publish(enemyTargetDirection)
